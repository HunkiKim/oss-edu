...
func DoRace(users []*user.User, turns int) {
	for _, u := range users {
		updateNumberOfTurns(u, turns)
	}
}
func updateNumberOfTurns(user *user.User, turns int) {
	user.NumberOfTurns = rand.Intn(turns + 1)
}

func CreateUsers(names string) ([]*user.User, error) {
	convertedNames, err := convertSlice(names)
	if err != nil {
		return nil, err
	}

	users := make([]*user.User, 0, len(convertedNames))

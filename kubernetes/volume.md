...
<img width="810" alt="image" src="https://github.com/HunkiKim/Mantech-Edu/assets/66348135/49420388-001d-4cf7-ac14-280a10c8cde4">

- PV는 Kubernetes 클러스터에서 사용 가능한 스토리지 리소스를 나타낸다. 
- PV는 네트워크 스토리지, 로컬 스토리지, 클라우드 공급자의 스토리지 등 다양한 유형의 스토리지를 포함할 수 있다. 
- PV는 클러스터 수준에서 생성되며, 클러스터 노드 또는 파드에 연결될 수 있는 스토리지를 제공한다.
  - Aws EBS같은 것들이 있다.

- PV는 스토리지 프로비저닝을 통해 동적으로 생성될 수도 있고, 사전에 수동으로 관리자에 의해 생성될 수도 있다. 
- PV는 용량, 액세스 모드(ReadWriteOnce, ReadOnlyMany, ReadWriteMany), 볼륨의 수명 주기 등의 속성을 가지고 있다.

## Persistent Volume Claim(PVC)
- PVC는 PV에 대한 요청을 정의하는 Kubernetes 오브젝트이다. 
- PVC는 PV의 일부 또는 전체 용량 및 액세스 모드와 같은 요구 사항을 명시한다. 
- PVC를 사용하여 애플리케이션에서 필요한 스토리지를 요청할 수 있으며, PVC는 동적으로 생성된 PV 또는 수동으로 생성된 PV와 연결된다.

- PVC는 애플리케이션의 스토리지 요구 사항을 추상화하고, 애플리케이션과 스토리지 간의 결합도를 낮춘다. 
  - 이는 애플리케이션을 스토리지 유형과 관리 방식으로부터 분리하여 유연성과 이식성을 제공한다. 
  - PVC는 파드 또는 볼륨 템플릿과 함께 사용될 수 있으며, PVC는 사용자가 요청한 요구 사항에 맞는 PV를 동적으로 할당받아 파드와 연결한다.

PV와 PVC는 Kubernetes에서 스토리지 관리를 간편하게 하고, 스토리지의 프로비저닝과 요청을 추상화하여 애플리케이션과 스토리지 간의 유연한 연결을 제공합니다.
...
kind: PersistentVolume
metadata:
  name: gcp-pv  # Persistent Volume의 이름을 지정합니다.
spec:
  capacity:
    storage: 1Gi  # 스토리지의 용량을 1GiB로 지정합니다.
  accessModes:
    - ReadWriteOnce  # 한 번에 한 노드에서 읽고 쓸 수 있도록 접근 모드를 설정합니다.
  storageClassName: gcp-storage  # 해당 PV에 연결할 스토리지 클래스의 이름을 지정합니다.
  gcePersistentDisk:
    pdName: gce-pd-1  # GCP의 Cloud Storage에서 생성한 버킷 이름을 입력합니다.
    fsType: ext4  # 파일 시스템 타입을 ext4로 지정합니다.
```
- gcp-storage-class
```yaml
apiVersion: storage.k8s.io/v1
...
- PD도 만들어야한다.
- PD : GCE PD (Google Compute Engine Persistent Disk)는 Google Cloud Platform (GCP)에서 제공하는 지속적인 블록 스토리지 서비스입니다. GCE PD는 가상 머신 (VM) 인스턴스에 연결하여 데이터를 저장하고 유지할 수 있는 영구적인 디스크입니다.
- GCE PD는 가상 머신의 데이터를 저장하는 데 사용되며, VM 인스턴스와 독립적으로 관리됩니다. GCE PD는 안정적이고 내구성이 있는 블록 스토리지이며, 데이터의 안전한 보관 및 전송을 제공합니다.
- GCE PD는 사용자가 원하는 용량으로 생성할 수 있으며, 필요에 따라 스냅샷을 생성하여 데이터를 백업하고 공유할 수도 있습니다. 또한, GCE PD는 가용 영역에서 생성되며, 여러 VM 인스턴스 간에 공유되지 않고 하나의 VM 인스턴스에만 연결됩니다.
- GCE PD는 Kubernetes에서 PV (Persistent Volume)의 백엔드 스토리지로 사용되기도 합니다. PV는 클러스터 내에서 스토리지를 추상화하여 Pod가 사용할 수 있는 지속적인 스토리지를 제공합니다.
